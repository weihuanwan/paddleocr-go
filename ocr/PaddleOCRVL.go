...
func (session *PaddleOCRVL) getLayoutParsingResults(layoutDetResult []*common.LayoutDetResult, originImage *gocv.Mat) []*PaddleOCRVLBlock {

	// 过滤
	filterLayoutDetResult := filterOverlapBoxes(layoutDetResult, "auto")

	paddleOCRVLBlocks := make([]*PaddleOCRVLBlock, 0, len(filterLayoutDetResult))
	for i := 0; i < len(filterLayoutDetResult); i++ {
		...
		if err != nil {
			fmt.Printf("crop error, index=%d, label=%s, err=%v\n", i, detResult.Label, err)
			continue
		}
		// 转化 base64
		base64Str, err := MatToBase64(cropImage)
		if err != nil {
			fmt.Printf("base64 encode error, index=%d, label=%s, err=%v\n", i, detResult.Label, err)
			continue

		}
		chatCompletionRequest := NewChatCompletionRequest(session.Model,
			base64Str,
			session.getTask(detResult.Label))
		chatCompletionResponse, err := session.Run(chatCompletionRequest)
		if err != nil {
			fmt.Printf("chat completion error, index=%d, label=%s, err=%v\n", i, detResult.Label, err)
			continue
		}

		ocrResult := chatCompletionResponse.Choices[0].Message.Content
		text := ocrResult
		// 判断是否是表格
		if detResult.Label == "table" {
			text = ConvertOtslToHtml(chatCompletionResponse.Choices[0].Message.Content)
		}

		paddleOCRVLBlock := &PaddleOCRVLBlock{
			LayoutDetResult: detResult,
			OcrResult:       ocrResult,
			Text:            text,
		}
		paddleOCRVLBlocks = append(paddleOCRVLBlocks, paddleOCRVLBlock)
	}
	return paddleOCRVLBlocks
}
